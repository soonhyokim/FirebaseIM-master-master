...
import afterapps.com.firebaseim.notificationlist.NotificationList;
...
public class ThreadActivity extends BaseActivity implements TextWatcher {
    ...
    private ArrayList<String> PastArrayList= new ArrayList<>();
    private ArrayList<String> NewArrayList = new ArrayList<>();
    private int count= 1;
    private int countnull = 1;
    ...
    protected void onStart() {
        ...
        if(PastArrayList == null && countnull == 1) {
            Log.v("ArrayList is", "empty");
            countnull = 0;
            return;
        }else {
            if (count == 1 && PastArrayList != null) {
                // 푸시키 다운
                mDatabase
                        .child("notifications")
                        .child("checked-users")
                        ...
                        .addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(DataSnapshot dataSnapshot) {
                                PastArrayList = (ArrayList<String>) dataSnapshot.getValue();
                                Log.v("PastArrayList", String.valueOf(PastArrayList));
                                NewArrayList = PastArrayList;
                                count = 0;
                ...
                //푸시키 저장
                NewArrayList.add(pushkey);
                Log.v("NewArrayList", String.valueOf(NewArrayList));
    ...
    public void notificationList(View view) {
        ...
        intent.putExtra("notificationList", data);
